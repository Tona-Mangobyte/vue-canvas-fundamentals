    ...
    setSpriteOffsets () {
      ...
      for (i = 0; i < this.sprites.length; ++i) {
        ...
        if (sprite.type === 'runner') {
          if (this.isRight) {
            sprite.offset -= this.spriteOffset // Move to the right or left
          } else {
            sprite.offset += this.spriteOffset // Move to the left or right
          }
          if (sprite.offset <= -780) {
            console.log('execute turnRight')
            sprite.track += 1 // Move to next track
            sprite.offset = 0
            if (sprite.track === 4) {
              console.log('execute start turnLeft')
              console.log('sprite.offset', sprite.offset)
              this.turnLeft()
              sprite.offset = 780
            }
            // sprite.offset = this.isRight ? 0 : -780 // Reset offset
            ...
            console.log('execute turnLeft', sprite.offset)
            sprite.track -= 1 // Move to previous track
            sprite.offset = -780
            if (sprite.track === 0) {
              console.log('execute start turnRight')
              console.log('sprite.offset', sprite.offset)
              this.turnRight()
              sprite.track = 1
              sprite.offset = -0
